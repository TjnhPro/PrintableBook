(() => {
  ...
  const renderFolderAssetWorkspace = (book, summary) => {
    ...
    const matchesStatus = (asset) => state.assetStatus === "All" || (state.assetStatus === "Active" ? effectiveInteriorAsset(book, asset).isActive : !effectiveInteriorAsset(book, asset).isActive);
    const matchesFrameMode = (asset) => state.assetFrameMode === "All" || effectiveInteriorAsset(book, asset).frameMode === state.assetFrameMode;
    const matching = allAssets.filter((asset) => `${valueFor(asset, "fileName", "")} ${valueFor(asset, "relativePath", "")}`.toLowerCase().includes(state.assetFilter.toLowerCase()) && (state.assetFolder === "All folders" || folderFor(asset) === state.assetFolder) && matchesStatus(asset) && matchesFrameMode(asset));
    ...
    const selectedOutsideCurrentView = selectedCount - selectedVisibleCount;
    const tile = (asset) => {
    ...
    const selectionCopy = selectedCount ? `${selectedCount} selected${selectedOutsideCurrentView ? ` · ${selectedOutsideCurrentView} outside this view` : ""}` : "Select artwork to apply a shared change.";
    return `<section class="interior-artwork-workspace"><header class="interior-artwork-heading"><div><h3>Interior artwork</h3><p>Review every available Book interior page, then control whether it is processed and which frame mode it uses.</p></div><p class="interior-artwork-count" role="status"><strong>${allAssets.length}</strong> artwork · <strong>${activeCount}</strong> active · <strong>${inactiveCount}</strong> inactive</p></header><div class="interior-artwork-filters"><label class="field asset-search-field"><span>Search artwork</span><input class="control" data-action="filter-assets" value="${escapeHtml(state.assetFilter)}" placeholder="File name or folder"></label><section class="asset-filter-controls" aria-label="Filter artwork"><span class="asset-filter-label">Filter artwork</span><div class="asset-filter-chip-groups"><div class="asset-folder-filter" role="group" aria-label="Interior artwork folder filters"><button class="${state.assetFolder === "All folders" ? "active" : ""}" data-action="asset-folder" data-asset-folder="All folders" aria-pressed="${state.assetFolder === "All folders"}">All folders</button>${folderNames.map((name) => `<button class="${state.assetFolder === name ? "active" : ""}" data-action="asset-folder" data-asset-folder="${escapeHtml(name)}" aria-pressed="${state.assetFolder === name}">${escapeHtml(name)}</button>`).join("")}</div><div class="asset-status-filter" role="group" aria-label="Interior artwork status filters">${["All", "Active", "Inactive"].map((name) => `<button class="${state.assetStatus === name ? "active" : ""}" data-action="asset-status" data-asset-status="${name}" aria-pressed="${state.assetStatus === name}">${name}</button>`).join("")}</div><div class="asset-frame-filter" role="group" aria-label="Interior artwork frame mode filters">${[["All", "All"], ["auto", "Auto"], ["enabled", "Frame"], ["disabled", "No frame"]].map(([value, label]) => `<button class="${state.assetFrameMode === value ? "active" : ""}" data-action="asset-frame-mode" data-asset-frame-mode="${value}" aria-pressed="${state.assetFrameMode === value}">${label}</button>`).join("")}</div></div></section></div><div class="interior-artwork-bulk-toolbar"><label class="artwork-select-all"><input type="checkbox" data-action="toggle-all-artwork" ${allShownSelected ? "checked" : ""} ${!eligibleMatching.length || controlsDisabled ? "disabled" : ""}> Select all ${eligibleMatching.length} shown</label><span class="artwork-selection-count" role="status">${selectionCopy}</span><button class="button-secondary" data-action="clear-artwork-selection" ${!selectedCount || controlsDisabled ? "disabled" : ""}>Clear selection</button><label class="field artwork-bulk-field"><span>Status</span><select class="control h-8" data-action="set-artwork-bulk-active"><option value="unchanged" ${state.assetBulkActive === "unchanged" ? "selected" : ""}>No change</option><option value="active" ${state.assetBulkActive === "active" ? "selected" : ""}>Active</option><option value="inactive" ${state.assetBulkActive === "inactive" ? "selected" : ""}>Inactive</option></select></label><label class="field artwork-bulk-field"><span>Frame mode</span><select class="control h-8" data-action="set-artwork-bulk-frame-mode"><option value="unchanged" ${state.assetBulkFrameMode === "unchanged" ? "selected" : ""}>No change</option><option value="auto" ${state.assetBulkFrameMode === "auto" ? "selected" : ""}>Auto</option><option value="enabled" ${state.assetBulkFrameMode === "enabled" ? "selected" : ""}>Frame</option><option value="disabled" ${state.assetBulkFrameMode === "disabled" ? "selected" : ""}>No frame</option></select></label><button class="button-primary" data-action="apply-artwork-bulk" ${bulkDisabled ? "disabled" : ""}>Apply to ${selectedCount} selected</button></div><p class="asset-result-count" role="status" aria-atomic="true">${matching.length} artwork shown</p><div class="interior-artwork-grid-scroll"><div class="interior-artwork-grid">${matching.length ? matching.map(tile).join("") : "<p class=\"empty-copy interior-artwork-empty\">No artwork matches this view.</p>"}</div></div></section>`;
  };
  ...
  content.addEventListener("click", (event) => {
    ...
    if (action === "toggle-all-artwork") {
      ...
      if (book && summary) {
        ...
        const shown = assetsFor(summary).filter((asset) => valueFor(asset, "kind", "") === "Interior" && `${valueFor(asset, "fileName", "")} ${valueFor(asset, "relativePath", "")}`.toLowerCase().includes(state.assetFilter.toLowerCase()) && (state.assetFolder === "All folders" || folderFor(asset) === state.assetFolder) && (state.assetStatus === "All" || (state.assetStatus === "Active" ? effectiveInteriorAsset(book, asset).isActive : !effectiveInteriorAsset(book, asset).isActive)) && (state.assetFrameMode === "All" || effectiveInteriorAsset(book, asset).frameMode === state.assetFrameMode) && !intro.sourceReferences.some((reference) => reference.toLowerCase() === String(valueFor(asset, "sourceReference", "")).toLowerCase()));
    ...
    if (action === "clear-artwork-selection") { state.selectedArtworkReferences.clear(); refreshInteriorArtworkWorkspace(); }
    ...
    if (action === "asset-folder") { state.assetFolder = target.dataset.assetFolder; state.artworkGridScrollTop = 0; refreshInteriorArtworkWorkspace(); }
    if (action === "asset-status") { state.assetStatus = ["Active", "Inactive"].includes(target.dataset.assetStatus) ? target.dataset.assetStatus : "All"; state.artworkGridScrollTop = 0; refreshInteriorArtworkWorkspace(); }
    if (action === "asset-frame-mode") { state.assetFrameMode = ["auto", "enabled", "disabled"].includes(target.dataset.assetFrameMode) ? target.dataset.assetFrameMode : "All"; state.artworkGridScrollTop = 0; refreshInteriorArtworkWorkspace(); }
