(() => {
  ...
  const renderBookTabs = (book, summary) => {
    ...
    if (state.selectedBookTab === "validation") body = `${panel("Validation checks", `<ul class="check-list">${checks.length ? checks.map((check) => `<li class="${valueFor(check, "isSuccess", false) ? "success" : "failure"}">${valueFor(check, "isSuccess", false) ? "✓" : "✕"}<span>${escapeHtml(valueFor(check, "message", ""))}</span></li>`).join("") : "<li class=\"empty-row\">No validation result yet.</li>"}</ul>`)}${coverCandidates.length > 1 ? panel("Cover selection", `<p class="panel-note">Choose the actual cover used by processing. The app will not guess from multiple candidates.</p><ul class="candidate-list">${coverCandidates.map((candidate) => `<li><span>${escapeHtml(candidate)}</span><button class="${candidate === selectedCover ? "button-primary" : "button-secondary"}" data-action="select-cover" data-book-id="${escapeHtml(bookId(book))}" data-cover-reference="${escapeHtml(candidate)}">${candidate === selectedCover ? "Selected" : "Use this cover"}</button></li>`).join("")}</ul>`, "mt-4") : ""}`;
  ...
  const renderOutputs = () => {
    ...
    content.innerHTML = `<div class="page-header"><div><h1>Outputs</h1><p>Generated, validated PDF artifacts.</p></div></div>${outputSummaries.length ? `<div class="output-grid">${outputSummaries.flatMap((summary) => valueFor(summary, "publishedArtifacts", []).map((artifact) => `<article class="output-card"><div class="pdf-mark">PDF</div><div><h2>${escapeHtml(artifact.split(/[\\/]/).pop())}</h2><p>${escapeHtml(valueFor(valueFor(summary, "bookId", {}), "value", ""))}</p><small>${dateTime(valueFor(summary, "lastRunAt", null))}</small></div></article>`)).join("")}</div>` : panel("Latest run", "<p class=\"empty-copy\">No published outputs discovered.</p>")}${panel("Previous runs", `<table class="data-table"><thead><tr><th>Book</th><th>Status</th><th>Cover / interior artifacts</th></tr></thead><tbody>${outputSummaries.length ? outputSummaries.map((summary) => `<tr><td>${escapeHtml(valueFor(valueFor(summary, "bookId", {}), "value", ""))}</td><td>${badge(workspaceStatus(summary))}</td><td>${valueFor(summary, "publishedArtifacts", []).length}</td></tr>`).join("") : "<tr><td colspan=\"3\" class=\"empty-row\">No history yet.</td></tr>"}</tbody></table>`, "mt-5")}`;
  };
  ...
  content.addEventListener("click", (event) => {
    ...
    if (action === "start-process") send("process.start", { bookIds: [...state.selectedBookIds], brandName: state.selectedBrand || brandSelect?.value || null });
