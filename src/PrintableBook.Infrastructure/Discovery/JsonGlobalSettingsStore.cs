...
public sealed class JsonGlobalSettingsStore(IApplicationRootDiscovery discovery, IFileSystem fileSystem) : IGlobalSettingsStore
{
    ...
    {
        var file = (await discovery.DiscoverAsync(cancellationToken)).Paths.SettingsFile;
        if (!await fileSystem.FileExistsAsync(file, cancellationToken)) return GlobalSettings.Default;
        return JsonSerializer.Deserialize<GlobalSettings>(await fileSystem.ReadTextAsync(file, cancellationToken), Options) ?? GlobalSettings.Default;
    }
    ...
    public async ValueTask SaveAsync(GlobalSettings settings, CancellationToken cancellationToken = default)
    {
        Validate(settings);
        var file = (await discovery.DiscoverAsync(cancellationToken)).Paths.SettingsFile;
        await fileSystem.WriteTextAtomicallyAsync(file, JsonSerializer.Serialize(settings, Options), cancellationToken);
    }
