...
test("desktop shell ships the Printable Book logo for its sidebar and window icon", () => {
  ...
  assert.match(page, /<img class="pb-brand-logo" src="assets\/printable-book-logo\.png" width="36" height="36" alt="" aria-hidden="true">/);
...
test("Brands displays certification state and validates the selected Brand", () => {
  ...
  assert.match(content.innerHTML, /2270 × 2270 px/);
  assert.match(content.innerHTML, /2588 × 2625 px/);
