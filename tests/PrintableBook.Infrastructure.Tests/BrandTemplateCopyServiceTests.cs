...
public sealed class BrandTemplateCopyServiceTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task CopyAsync_creates_templates_directory_and_copies_exactly_the_two_required_psd_files()
    {
        var (service, brand, workspace) = await CreateScenarioAsync("cover-v1", "app-v1");
        ...
        Assert.Equal(["cover.psd", "app_plus.psd"], result.CopiedFileNames);
        ...
        Assert.Equal(2, Directory.GetFiles(result.DestinationDirectory.Value).Length);
    }
    ...
    {
        var (service, brand, workspace) = await CreateScenarioAsync("cover-v1", "app-v1");
        File.Delete(Path.Combine(brand.Value, "app_plus.psd"));
    ...
    private async Task<(BrandTemplateCopyService Service, DirectoryReference Brand, BookWorkspace Workspace)> CreateScenarioAsync(string cover, string appPlus)
    {
