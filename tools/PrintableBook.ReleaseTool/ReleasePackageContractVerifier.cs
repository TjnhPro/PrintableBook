...
internal sealed class ReleasePackageContractVerifier
{
    ...
    public void Verify(string archivePath, Version version, string runtimeIdentifier)
    {
        ...
        var entries = archive.Entries.Where(entry => !entry.FullName.EndsWith("/", StringComparison.Ordinal)).Select(entry => Normalize(entry.FullName)).ToArray();
        var roots = entries.Select(entry => entry.Split('/')[0]).Distinct(StringComparer.Ordinal).ToArray();
        ...
        {
            var path = wrapped ? entry[(wrapper.Length + 1)..] : entry;
