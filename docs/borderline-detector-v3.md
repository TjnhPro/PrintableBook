...
BorderLine V3 reads the canonical source once. It evaluates pass 1 at the configured shallow depth, then evaluates the deeper pass only when pass 1 has no coherent four-sided frame. BorderPixel V1 runs only when BorderLine is negative and keeps its exact perimeter-pixel algorithm.
