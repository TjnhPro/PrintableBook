...
The corpus images, the generated measurement report, debug overlays, and the reviewed-frame manifest remain local-only. They are never committed or uploaded. Run the corpus with the command in the [README](../README.md#local-artwork-corpus), inspect the magenta overlays, and update the reviewed geometry manifest before accepting a new positive-artwork certification.
